...
using KangarooSolver.GoalsOther;
using Rhino.Geometry;
using SpatialSlur.SlurRhino;

namespace K2Collisions.K2CollisionSolver
{
    public class K2CollisionsComponent : GH_Component
    {
        ...
        public override Guid ComponentGuid { get { return new Guid("{ccc263df-3388-4886-a74e-511176bd0afc}"); } }

        public K2CollisionsComponent()
        ...
        {
            pManager.AddGenericParameter("O", "O", "O", GH_ParamAccess.list);
        }
        ...
        private void SpatialGridCollisions(List<double> goalValues)
        {
            ...
            for (int i = 0; i < _edges.Length; i++)
            {
                ...
                _grid.Search(bbox, foundIds =>
                {
                    ...
                    {
                        if (j <= i || meshIndex[j] == meshIndex[i]) continue;
                        a.Add(_edges[i][0]);
                        b.Add(_edges[i][1]);
                        c.Add(_edges[j][0]);
                        d.Add(_edges[j][1]);
        ...
        private void springsGoals(List<double> goalValues)
        {
            ...
            for (int i = 0; i < clothedE.Count; i++)
                goals.Add(new SpringGrow(clothedS[i], clothedE[i], PS.GetPosition(clothedS[i]).DistanceTo(PS.GetPosition(clothedE[i])), goalValues[0], goalValues[2]));

            for (int i = 0; i < nakedS.Count; i++)
                goals.Add(new SpringGrow(nakedS[i], nakedE[i], PS.GetPosition(nakedS[i]).DistanceTo(PS.GetPosition(nakedE[i])), goalValues[1], goalValues[2]));
        ...
        public override void DrawViewportWires(IGH_PreviewArgs args)
        {
            ...
            args.Display.DrawLines(lines, Color.Black, 1);
        }
        ...
        public override void DrawViewportMeshes(IGH_PreviewArgs args)
        {
            ...
            if (meshes != null)
                foreach (PlanktonMesh m in meshes)
                {
                    Mesh tempMesh = RhinoSupport.ToRhinoMesh(m);
                    args.Display.DrawMeshShaded(tempMesh, material);
                }
        }
